...
# This package only allows certain packages to be installed as Lambda Layers
ALLOWED_LAYER_PACKAGES = ["boto3", "moviepy"]
...
def create_layer_zip(name, packages, s3_bucket, s3_key):
    ...
    for p in packages:
        if p.get("name") in ALLOWED_LAYER_PACKAGES:
            ...
            print(f"Package {p.get('name')} is not allowed. Skip.")
