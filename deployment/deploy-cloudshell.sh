...
echo "cdk deploy --parameters inputUserEmails=${CDK_INPUT_USER_EMAILS} --requires-approval never  --all"
# Deploy CDK package - this step will launch one CloudFormation stack with three nested stacks for different sub-systems.
cdk deploy --parameters inputUserEmails=${CDK_INPUT_USER_EMAILS} --requires-approval never  --all