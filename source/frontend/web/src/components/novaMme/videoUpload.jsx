...
import EmbeddingSetting from './embedSetting'
...
class VideoUpload extends React.Component {
    ...
    getMediaType(filename) {
        ...
        for (const [mediaType, extensions] of Object.entries(this.supportedFileTypes)) {
    ...
    handleNavigate = e => {
        if(e.detail.requestedStepIndex === 1) {
            ...
            const setting = this.mmEmbedSettingRef.current.getRequest();
            this.setState({alert: null, request: setting})
        }
            ...
            <div className="videoupload">
                ...
                <Wizard
                    i18nStrings={{
                        ...
                        nextButton: "Next",
                        submitButton: "Upload a file and start analysis",
                        optional: "optional"
                    }}
                    ...
                    steps={[
                        {
                            ...
                            content: (
                                ...
                                <FileUpload
                                    ...
                                    tokenLimit={3}
                                    multiple={false}
                                    constraintText={`Supported formats — ${Object.entries(this.supportedFileTypes).map(([k,v]) => `${k}: .${v.join(", .")}`).join(", ")}`}
                                    accept='.txt,.png,.jpeg,.gif,.webp,.mp3,.wav,.ogg,.mp4,.mov,.mkv,.webm,.flv,.mpeg,.mpg,.wmv,.3gp'
                                />
                                </Container>                                
                            )
                        },
                        ...
                        content: (
                            <SpaceBetween direction="vertical" size="l">
                              <EmbeddingSetting modality={this.getMediaType(this.state.uploadFiles[0]?.name)} ref={this.mmEmbedSettingRef} />
                            </SpaceBetween>
                        ),
                        },
                        {
                            title: "Upload a file and start analysis",
